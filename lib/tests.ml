...
let test_garden ?(valid = true) nb_flowers params =
  let open Random in
  let generate = if valid then Correct.gen_flower else gen_flower in
...
let lifeform ?(classical = true) ?(logpoll = false) ?(print = false) ?(printer = None) (fs : Engine.Fo.form list) =
  let open Utils in
  ...
  Printf.printf
    "%n/%n (%.2f%%) valid flowers"
    !nb_valid
    nb_total
    ((float_of_int !nb_valid) /. (float_of_int nb_total) *. 100.)
